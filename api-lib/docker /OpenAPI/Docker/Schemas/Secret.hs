...
module OpenAPI.Docker.Schemas.Secret where
...
data Secret = Secret
    {
        ...
        spec :: (Data.Maybe.Maybe (SecretSpec)),
        updatedAt :: (Data.Maybe.Maybe (Data.Text.Text)),
        version :: (Data.Maybe.Maybe (ObjectVersion))
    }
    deriving (Show)

instance Data.Aeson.ToJSON Secret where
    toJSON Secret {..} = Data.Aeson.object
        ([ 
        ]
    ...
    toEncoding Secret {..} = Data.Aeson.Encoding.pairs
...
instance Data.Aeson.FromJSON Secret where
    parseJSON = Data.Aeson.withObject "Secret" $ \o ->
        Secret
